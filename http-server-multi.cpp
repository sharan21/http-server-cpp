...
{
	/* Used by a thread to serve its particular client in its FD set */

	int client_fd = all_fds[thread_number][index].fd, rec_buffer_size;
...
int main()
{
	...
	while (true)
	{
		accept_client_connection(client_addr);
			cout << "Thread workload: " << endl;
			int sum =0;
			for(int i=0;i<10;i++){
				sum+= curr_no_of_clients[i];
				cout<< "Thread " << i << " : " <<  curr_no_of_clients[i]<< " clients" << endl;
			}
			cout<< endl << "Total # clients: "<< sum << endl;
	}
	return 0;
}